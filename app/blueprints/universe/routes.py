...
def _prepare_chart_payload(cumulative_growth: pd.DataFrame) -> dict[str, object]:
    ...
    df = cumulative_growth.copy().ffill().dropna(how="all")
    if df.empty:
        return {"series": []}

    # Rebase to the first visible date so the benchmark always starts at 100.
    ...
    # If still very long, thin evenly to avoid overloading the client.
    if len(df) > 1500:
        step = len(df) // 1200 + 1
        df = df.iloc[::step]
...
def investment_universe():
    ...
    try:
        ...
        if df.empty:
            ...
            cumulative_growth = np.exp(df.cumsum())
